...
if errorlevel 1 (
    echo [docvault-ai] server not running - starting in background ...
    ...
    echo [docvault-ai] failed to start the docvault server.
    echo Check the log at ^<vault^>\logs\server.log ^(default C:\docvault-data\logs\server.log^),
    echo or run ".venv\Scripts\docvault.exe serve" from a console to see the error.
    pause
    exit /b 3
)
:ready
