...
def _uvicorn_log_config(log_file: Path) -> dict:
    """uvicorn's default log config plus a rotating file handler, so a server
    launched headless (pythonw, no console) still leaves diagnostics behind."""
    import copy
    ...
    # "uvicorn.error" propagates to "uvicorn", so only the parent gets the file
    # handler; "uvicorn.access" has propagate off and needs its own.
    for name in ("uvicorn", "uvicorn.access"):
        logger = log_config["loggers"].setdefault(name, {"level": "INFO"})
...
def install_windows() -> None:
    ...
    typer.echo("Setup / update / removal are handled by the PowerShell scripts in windows\\:")
    typer.echo("  install everything:   powershell -ExecutionPolicy Bypass -File windows\\setup.ps1")
    typer.echo("  update after git pull: powershell -ExecutionPolicy Bypass -File windows\\update.ps1")
    typer.echo("  uninstall:            powershell -ExecutionPolicy Bypass -File windows\\uninstall.ps1")
    typer.echo("  context menu only:    windows\\install-context-menu.ps1 / uninstall-context-menu.ps1")
