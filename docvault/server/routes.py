...
def build_router(cfg: Config) -> APIRouter:
    ...
    def get_config() -> ConfigOut:
        return ConfigOut(
            ...
            suggested_tags=list(cfg.ingest.suggested_tags),
            trash_retention_days=cfg.trash.retention_days,
            cleanup_retention_days=cfg.cleanup.retention_days,
        )
    ...
    def delete_doc(sha256: str, body: DeleteIn) -> OkOut:
        ...
        # Move the file first (when requested), the metadata entry last: as
        # long as the entry exists, the file is discoverable. If the file
        # move fails, the record stays intact and the error surfaces.
